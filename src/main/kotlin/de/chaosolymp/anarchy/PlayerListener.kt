...
class PlayerListener(private val plugin: AnarchyPlugin) : Listener {
    ...
    fun handleJoin(event: PlayerJoinEvent) {
        ...
        this.plugin.databaseManager.insertIntoTableIfNotExists(player.uniqueId, player.name);
        this.plugin.databaseManager.incrementJoins(player.uniqueId);

        event.joinMessage = this.plugin.messageConfiguration.getMessage(
            "join", arrayOf(
    ...
    fun handleQuit(event: PlayerQuitEvent) {
        event.quitMessage = this.plugin.messageConfiguration.getMessage(
            "quit", arrayOf(
                Replacement("player", event.player.name)
            )
        )
    ...
    fun handleDeath(event: PlayerDeathEvent) {
                ...
                if (player.uniqueId == event.entity.uniqueId) {
                    player.sendMessage(
                        this.plugin.messageConfiguration.getMessage(
                            "death.suicide.personal", arrayOf(
        ...
        when (event.entity.lastDamageCause?.cause) {
            EntityDamageEvent.DamageCause.SUICIDE -> event.deathMessage = this.plugin.messageConfiguration.getMessage(
                "death.suicide",
                arrayOf(Replacement("player", event.entity.name))
            )
            EntityDamageEvent.DamageCause.FALL -> event.deathMessage = this.plugin.messageConfiguration.getMessage(
                "death.fall-damage",
                arrayOf(Replacement("player", event.entity.name))
            )
        ...
        event.entity.killer?.uniqueId?.let { it ->
            this.plugin.databaseManager.incrementKills(it);
            val opt = this.plugin.databaseManager.getPlayerStatistic(it);
            if (opt.isPresent) {
                val statistic = opt.get()
                if (statistic.killStreak > 0 && statistic.killStreak % this.plugin.pluginConfiguration.killStreakJump == 0) {
                    Bukkit.broadcastMessage(
                        this.plugin.messageConfiguration.getMessage(
                            "killstreak.global", arrayOf(
